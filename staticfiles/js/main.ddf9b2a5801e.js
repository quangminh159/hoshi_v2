...
function initializeCommentForms() {
    ...
    document.querySelectorAll('.add-comment-form').forEach(form => {
        ...
        if (form.getAttribute('data-initialized') === 'true') {
            console.log(`Form với post-id=${form.getAttribute('data-post-id')} đã được khởi tạo trước đó.`);
            return;
        }
        
        // Đánh dấu form đã được khởi tạo
        form.setAttribute('data-initialized', 'true');
        console.log(`Đăng ký event submit cho form với post-id=${form.getAttribute('data-post-id')}`);
        
        // Remove any existing submit event handlers first
...
function initializeReplyButtons() {
    ...
    document.querySelectorAll('.reply-button').forEach(button => {
        ...
        const buttonId = `reply-button-${button.getAttribute('data-post-id')}-${button.getAttribute('data-username')}`;
...
function addCommentToDOM(comment, postId, isReply, parentId) {
    // Kiểm tra xem comment đã tồn tại trong DOM chưa
